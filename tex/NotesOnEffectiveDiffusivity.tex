...
%For simplicity, we assume x-periodic geometry.  First, we recall that $\dd \vth = 0$ along the contour $\Th$, hence
%\beq
%\frac{\p A}{\p t} = - \frac{\p \vth / \p t}{\p \theta / \p A}\Big)_{\Th}\per
%\eeq
...
\beq
\label{KeDefn}
K_e (A, t) =  \frac{ \kappa \oint_\gamma |\nabla\vth| \dd l }{\p \vth/\p A} 
...
\beq
\label{knak}
\kappa_{N}(\Th,t) = \frac{K_e}{L_{min}^2} = \frac{L_{eq}^2}{L_{min}^2}\kappa\com
\eeq
